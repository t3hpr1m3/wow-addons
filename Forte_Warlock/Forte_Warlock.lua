...
if FW.CLASS == "WARLOCK" then
	...
	if ST then
		...
		ST:RegisterSpell(  348,	1,015,1,ST.DEFAULT); -- Immolate
			...
			ST:RegisterSpellModTlnt(348,851050,1,6); -- Inferno
		...
		ST:RegisterSpell(  980,	1,024,1,ST.SHARED2); -- Bane of Agony
			...
			ST:RegisterSpellModGlph(980, 56241, 4);
	...
	FW:SetMainCategory(FWL.SELF_MESSAGES);
		FW:SetSubCategory(FWL.BLOOD_PACT,FW.ICON.BASIC,2)
			...
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.BLOOD_PACT_GAIN,		"",		"BloodpactGainColor");
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.BLOOD_PACT_LOSS,		"",		"BloodpactLossColor");
	...
	if CD then
		...
		FW.InstanceDefault.Cooldown.Filter[FW:SpellName(47541)] = 		{nil,nil,{-2,0.00,0.63,0.05}};--death coil
		...
		FW.InstanceDefault.Cooldown.Filter[FW:SpellName(34935)] = 		{nil,nil,{-1}};--backlash
	end
end
