local revision = tonumber(string.sub("$Revision: 166 $", 12, -3)) or 1
...
--~ 			end
	...
	if not InCombatLockdown() then
		if not layout.clickthrough and playerunit[self.parent.layout.target] and (data.type == "BUFF" or data.type == "TENCH") then
			local SAB = self.SAB
			if not SAB then
			...
			if data.type == "BUFF" then
				SAB:SetAttribute("*spell2", data.realname)
				SAB:SetAttribute("*target-slot2", nil);
			else -- data.type == "DEBUFF"
				SAB:SetAttribute("*spell2", nil)
...
function EBB_Bar.prototype:RecycleSAB()
	if self.SAB then
