...
ElkBuffBars.revision = tonumber(string.sub("$Revision: 167 $", 12, -3)) or 1
...
function ElkBuffBars:AddKnownName(type, name)
	if self.knownnames[type] and not self.knownnames[type][name] then
...
function ElkBuffBars:ScanData_TENCH_Helper(...)
	...
	for itemIndex = 1, numItems do
		...
		if ( hasEnchant ) then
			local enchantExpiration = enchantExpiration and (enchantExpiration / 1000) or nil
...
local function hasTEnchUpdate(...)
	...
	for itemIndex = 1, numItems do
		local hasEnchant, enchantExpiration, enchantCharges = select(RETURNS_PER_ITEM * (itemIndex - 1) + 1, ...)
		local offset = (itemIndex - 1) * 3
		if (hasEnchant ~= TEnchBuffer[offset + 1]) or (enchantExpiration or 0) > (TEnchBuffer[offset + 2] or 0) or (enchantCharges ~= TEnchBuffer[offset + 3]) then
			changes = true
			break
		end
	end
	table.wipe(TEnchBuffer)
	for i = 1, numVals do
...
function ElkBuffBars:OnTooltipUpdate()
	...
	for k, v in ipairs(self.bargroups) do
		cat:AddLine(	"text",		tostring(k)..". "..(v.layout.anchortext or "???"),
						...
						"hasCheck",	true,
						"checked",	v.layout.configmode,
						"func",		"ToggleConfigMode",
						"arg1",		v
					)
	end
end
...
function delayed_code()
	...
	typeoverride_validate = {BUFF = "BUFF", DEBUFF = "DEBUFF", TENCH = "TENCH", [""] = L["OPTIONS_OVERRIDES_TYPE_VALIDATE_DEFAULT"]}
	...
	if not groupoptionscache[id] then
		groupoptionscache[id] = {
			type = "group", name = (L["OPTIONS_GROUP_NAME"]):format(id), desc = (L["OPTIONS_GROUP_DESC"]):format(id), args = {
				configmode = {
					type = "toggle", name = L["OPTIONS_GROUP_CONFIG_NAME"], desc = L["OPTIONS_GROUP_CONFIG_DESC"],
					get = function() return ElkBuffBars.db.profile.bargroups[id].configmode end,
					set = function() ElkBuffBars.bargroups[id]:ToggleConfigMode() end,
					order = 101,
				},
				anchorshown = {
