...
function FW:Popup1975(update)
	local txt = "|cff00ff00This version introduces 'Options linking'!\n\nYou can choose to link all your profiles and clones together now, and link the options that are the same everywhere. You can also link every new profile or clone in the future. You can always change or re-run this at 'Advanced Options > Options linking'.|r\n\n|cffffcc00Do you want to smart link everything now?|r";
	if update then
...
function FW:InitFrameVars(frame) -- also contains compatibility fixes (these should be removed at some point, e.g. ver2.00)
	...
	if FW.Settings.GlobalFrameNames then
		_G[frame.name] = frame;
		if frame.name == "FX_Timer1" then -- some compatibility stuff
			_G["FX_Timer"] = frame;
		elseif frame.name == "FX_Cooldown1" then
			_G["FX_Cooldown"] = frame;
		elseif frame.name == "FX_Splash1" then
			_G["FX_Splash"] = frame;
		end
	end
end
...
function FW:InstanceNameToIndex(name,root,case) -- case insensitive, currently used for slash command only
	if case then
...
FW.Default = {
	...
	OptionsSubHeaderTexture = "Interface\\AddOns\\Forte_Core\\Textures\\Smooth",
};
