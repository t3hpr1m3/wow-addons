...
local function ScrollTo(obj)
	FW:ScrollTo(obj.title,1);
end
...
function FW:SmartLinkAll()
	...
	FW:BuildOptions();
	FW:RefreshOptions();
	FW:Show("ForteXorcist: All profiles and clones are now linked, and options with the same settings are linked as well. In addition, future new profiles and clones will be linked automatically too. You can change automatic linking at 'Advanced Options > Options Linking'.",0,1,0);
end
...
local function OutputOption(from,name)
	if type(from[name]) == "table" then
		local m = "";
		for k, v in pairs(from[name]) do
			if type(v) == "table" then
				return "too complex to display";
			else
				m = m .. "\n".. convertValue(v);
			end
		end
...
local function NewDefaultButton(parent)
	...
	obj.SetEnabled = function(self,enabled)
		if enabled and not (self.parent.s and MatchOption( self.parent.s, self.parent.d, self.parent.o)) then
			obj.title = FWL.DEFAULT;
			obj.tip = FWL.DEFAULT_TT.."\n\n|cffffffffDefault:|r"..OutputOption(self.parent.d,self.parent.o);
			self:Enable();
		else
			self:Disable();
		end
...
function FW:ScrollTo(v,fo,instance)
	...
	if Anchors[v] then
		...
		if refresh then
			FW:BuildOptions();
			FW:RefreshOptions(); -- for tab color only
		end
	end
end
...
local function EditboxFix() -- fixes 4.1 editbox problem on show
	local current = _G.FWOptions.show;
	_G.FWOptions:Hide();
	_G.FWOptions:Show();
	_G.FWOptions.show = current;
end
...
do
	...
	function FW:RegisterOption(typ,width,pos,text,tip,option,func,minimum,maximum,enabled)
		...
		if not o then
			...
			tinsert(FW_Options[mc][MAIN_DATA_TABLE][sc][SUB_DATA_TABLE],o,{typ,width,pos,text,tip,option,func,minimum,maximum,enabled});
		end
	end
end
...
local function FC_SetOptionsFont()
	...
	_G.FWOptionsHeaderTitle:SetFont(foh,sih,flh);
		...
		for sub_index, sub_data in ipairs(main_data[MAIN_DATA_TABLE]) do -- SUB OPTION LEVEL
			...
			for i, d in ipairs(sub_data[SUB_DATA_TABLE]) do -- OPTION LEVEL
				...
				if d[1] and d[1] ~= FW.NIL then
					...
					if d[1] == FW.URL or d[1] == FW.FIL or d[1] == FW.MSG or d[1] == FW.NUM or d[1] == FW.MS2 or d[1] == FW.PRO  or d[1] == FW.NU2 then
...
local function FC_SetOptionsColor()
	...
	_G.FWOptionsHeaderBackground:SetVertexColor(r,g,b,a);
	...
	if extrabutton then
		extrabutton.texture:SetVertexColor(r,g,b,1);
	end
	a = 0.8;
...
local function FC_SetOptionsBackdrop()
	_G.FWOptionsHeaderBackground:SetTexture(FW.Settings.OptionsHeaderTexture);
	...
	if extrabutton then
		extrabutton.texture:SetTexture(FW.Settings.OptionsHeaderTexture);
	end
	FC_SetOptionsColor();
end

function FW:HideAllOptions()
	for main_index, main_data in ipairs(FW_Options) do -- MAIN OPTION LEVEL
...
function FW:HideUnneededOptions()
	FW:HideAllOptions(); -- fixes 4.1 editbox problem on scroll
...
function FW:BuildOptions()
			...
			for i, d in ipairs(sub_data[SUB_DATA_TABLE]) do -- OPTION LEVEL
				...
				if d[1] and d[1] ~= FW.NIL then
					if  d.option then
						...
						if instanceof then
							d.option = createOption[d[1]](f, d[6], root.Instances[root.Active], FW.InstanceDefault[instanceof]);
						else
	...
	if x ~= 0 then
		extrabutton = extrabutton or NewOptionsHeader(_G.FWOptions);
...
function FW:LocalizedData()
	...
	FW:SetMainCategory(FWL.GENERAL,FW.ICON.DEFAULT,1,"DEFAULT","FWOptions",nil,tab_actions);
		...
		FW:SetSubCategory(FWL.GENERAL_MO,FW.ICON.DEFAULT,2,FW.EXPAND);
		...
		FW:SetSubCategory(FWL.GENERAL_MA,FW.ICON.APPEARANCE,3,FW.EXPAND);
	...
	FW:SetMainCategory(FWL.RAID_MESSAGES,FW.ICON.MESSAGE,10,"RAIDMESSAGES");
		...
		FW:SetSubCategory(FWL.WHERE_TO_SHOW,FW.ICON.DEFAULT,1);
	...
	FW:SetMainCategory(FWL.ADVANCED,FW.ICON.DEFAULT,99,"DEFAULT");
		...
		FW:SetSubCategory(FWL.GENERAL_MO,FW.ICON.DEFAULT,1);
			...
			FW:RegisterOption(FW.NUM,1,FW.NON,FWL.GENERAL_MO4,	FWL.GENERAL_MO4_TT,	"FrameSnapDistance",nil,nil,nil,"FrameSnap");
			FW:RegisterOption(FW.NUM,1,FW.NON,FWL.GENERAL_MO5,	FWL.GENERAL_MO5_TT,	"FrameDistance",nil,nil,nil,"FrameSnap");
			FW:RegisterOption(FW.CHK,1,FW.NON,FWL.SHOW_STARTUP,FWL.SHOW_STARTUP_TT,	"ShowStartupText");
		...
		FW:SetSubCategory(FWL.OPTIONS_BEHAVIOR,FW.ICON.DEFAULT,1);
			...
			FW:RegisterOption(FW.CHK,1,FW.NON,FWL.EXPAND_ALL,		FWL.EXPAND_ALL_TT,		"ExpandSubcats",ResetExpanded);
			FW:RegisterOption(FW.CHK,1,FW.NON,FWL.LOAD_EXPANDED,	FWL.LOAD_EXPANDED_TT,	"LoadExpandSubcats",ResetExpanded);
			FW:RegisterOption(FW.CHK,1,FW.NON,FWL.ANIMATE_SCROLL,	FWL.ANIMATE_SCROLL_TT,	"AnimateScroll");
			
		FW:SetSubCategory(FWL.OPTIONS_LINKING,FW.ICON.DEFAULT,1);
			FW:RegisterOption(FW.INF,2,FW.NON,	FWL.LINK_HINT1);
			FW:RegisterOption(FW.INF,2,FW.NON,	FWL.LINK_HINT2);
			
			FW:RegisterOption(FW.CO2,1,FW.NON,FWL.LINKED_NONE,"",		"LinkNoneColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.LINKED_PROFILES,"",	"LinkProfileColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.LINKED_CLONES,"",		"LinkCloneColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.LINKED_BOTH,"",		"LinkBothColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.BTN,1,FW.NON,FWL.CROSS_PROFILE_LINK,	FWL.CROSS_PROFILE_LINK_TT,"",AutoLinkProfiles);
			FW:RegisterOption(FW.BTN,1,FW.NON,FWL.CROSS_CLONE_LINK,		FWL.CROSS_CLONE_LINK_TT,"",AutoLinkClones);
			
			FW:RegisterOption(FW.CO2,1,FW.NON,FWL.DIFFERS_NONE,"",		"DiffNoneColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.DIFFERS_PROFILES,"",	"DiffProfileColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.DIFFERS_CLONES,"",	"DiffCloneColor",	FW_RefreshOptionsNoStyleThrottle);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.DIFFERS_BOTH,"",		"DiffBothColor",	FW_RefreshOptionsNoStyleThrottle);
			--FW:RegisterOption(FW.BTN,1,FW.NON,FWL.CROSS_PROFILE_CORRECT,FWL.CROSS_PROFILE_CORRECT_TT,	"",	AutoFixProfiles);
			--FW:RegisterOption(FW.BTN,1,FW.NON,FWL.CROSS_CLONE_CORRECT,	FWL.CROSS_CLONE_CORRECT_TT,		"",	AutoFixClones);
			
			FW:RegisterOption(FW.CHK,1,FW.NON,FWL.ALWAYS_LINK_PROFILE,	FWL.ALWAYS_LINK_PROFILE_TT,	"LinkProfile");
			FW:RegisterOption(FW.CHK,1,FW.NON,FWL.ALWAYS_LINK_CLONE,	FWL.ALWAYS_LINK_CLONE_TT,	"LinkClone");

		FW:SetSubCategory(FWL.GENERAL_OA,FW.ICON.APPEARANCE,1);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.GENERAL_OA1,		FWL.GENERAL_OA1_TT,	"OptionHeaderColor",		FC_SetOptionsColor);
			FW:RegisterOption(FW.COL,1,FW.NON,FWL.GENERAL_OA2,		FWL.GENERAL_OA2_TT,	"OptionBackgroundColor",	FC_SetOptionsColor);
			...
			FW:RegisterOption(FW.TXT,2,FW.LAS,FWL.GENERAL_OA3,		"",					"OptionsHeaderTexture" , 	FC_SetOptionsBackdrop);
			FW:RegisterOption(FW.TXT,2,FW.LAS,FWL.SUB_HEADERS,		"",					"OptionsSubHeaderTexture" , FC_SetOptionsBackdrop);
		...
		FW:SetSubCategory(FWL.CORE,FW.ICON.DEFAULT,1);
			...
			FW:RegisterOption(FW.BTN,1,FW.NON,FWL.REMOVE_BACKUP,	FWL.REMOVE_BACKUP_TT,	"",FW_ClearBackup,nil,nil,function() return FW:Size(FC_Saved) > 0; end);
