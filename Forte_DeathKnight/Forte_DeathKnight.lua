...
if FW.CLASS == "DEATHKNIGHT" then
	...
	if ST then
		...
		ST:RegisterSpell(55095,	1,021,1,ST.DEFAULT); -- Frost Fever
			...
			ST:RegisterSpecialRefresh(55095); -- Festering Strike
		...
		ST:RegisterSpell(59879,	1,021,1,ST.DEFAULT); -- Blood Plague
			...
			ST:RegisterSpecialRefresh(59879); -- Festering Strike
		...
		do
			...
			ST:RegisterManualSpellOfFlag(aotd,ST.PET);
			...
			local function DK_CombatLogEvent(event,...)
				if select(5,...) == PLAYER and select(2,...) == "SPELL_SUMMON" and select(13,...) == aotd then
					...
					if i then
						...
						ST.ST:insert(startTime+duration,0,duration,aotd,0,ST.PET,icon,aotd,0,0,"none",0,ST.PRIOR_NONE,0,1,aotd_active,0,00000,0,startTime+duration,duration,1.0,0);
					end
		...
		local function SH_TotemUpdate(event,index)
			...
			if name ~= "" then
				ST:RegisterManualSpellOfFlag(name,ST.PET);
				ST.ST:insert(startTime+duration,0,duration,name,0,ST.PET,icon,name,2,0,"none",0,ST.PRIOR_NONE,0,1,0,0,00000,0,startTime+duration,duration,1.0,0);
			end
	...
	if CD then
		...
		FW:SetMainCategory(FWL.COOLDOWN_TIMER);
			FW:SetSubCategory(FWL.MY_COOLDOWNS);
				FW:RegisterOption(FW.CO2,1,FW.NON,FWL.RUNE_BLOOD,			"",	"RuneBloodColor",	CD.FilterChange);	
				FW:RegisterOption(FW.CO2,1,FW.NON,FWL.RUNE_UNHOLY,			"",	"RuneDeathColor",	CD.FilterChange);	
				FW:RegisterOption(FW.CO2,1,FW.NON,FWL.RUNE_FROST,			"",	"RuneFrostColor",	CD.FilterChange);	
		
		FW.InstanceDefault.Cooldown.RuneBloodColor = {[0]=true,1.00,0.00,0.00};
		FW.InstanceDefault.Cooldown.RuneDeathColor = {[0]=true,0.20,0.80,0.00};
		FW.InstanceDefault.Cooldown.RuneFrostColor = {[0]=true,0.00,0.43,1.00};
		
		CD:AddCooldownFlag("RuneBloodColor");
		CD:AddCooldownFlag("RuneDeathColor");
		CD:AddCooldownFlag("RuneFrostColor");
	end
end
