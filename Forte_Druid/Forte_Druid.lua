...
if FW.CLASS == "DRUID" then
	...
	if ST then
		...
		ST:RegisterBuff(16880); -- Nature's Grace
		...
		ST:RegisterBuff(90164); -- Astral Alignment Balance 4P T11 bonus
		ST:RegisterBuff(90166); -- Strength of the Panther Feral 4P T11 bonus
	...
	if CA then
		...
		FW:SetMainCategory(FWL.RAID_MESSAGES);
			...
			FW:SetSubCategory("Self Damage Reduction",FW.ICON.SPECIFIC,2);
				FW:RegisterOption(FW.MS2,2,FW.NON,bs,	 "",	"BSStart"); 
				FW.Default.BSStart = {[0]=1,"+++ Barkskin (10 sec) +++"};

				FW:RegisterOption(FW.MS2,2,FW.NON,si,	"",	"SIStart"); 
				FW.Default.SIStart = {[0]=1,"+++ Survival Instincts (12 sec) +++"};

				FW:RegisterOption(FW.MS2,2,FW.NON,fr,	"",	"FRStart"); 
				FW.Default.FRStart = {[0]=1,"+++ Frenzied Regeneration (20 sec) +++"};
				
			FW:SetSubCategory("Other",FW.ICON.SPECIFIC,2);
				FW:RegisterOption(FW.MS2,2,FW.NON,rebirth,	"",	"RebirthStart"); 
				FW.Default.RebirthStart = {[0]=1,">>> Rebirth on %s <<<"};

				FW:RegisterOption(FW.MS2,2,FW.NON,innervate,	"",	"InnervateStart"); 
				FW.Default.InnervateStart = {[0]=1,">>> Innervate on %s <<<"};

				FW:RegisterOption(FW.MS2,2,FW.NON,tranquility,	 "",	"TranquilityStart"); 
