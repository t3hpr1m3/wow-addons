	...
	if not CA_IsChannel[spellName] then -- if marked as special channel it's already taken care off
		...
		if ST and ST.Drain[spellName] then
			...
			if i then
				...
				ST.ST:insert(endTime,FW:CastTime(spell),realduration,tar,0,ST.DRAIN,texture,spellName,targettype,0,guid,0,id,0,1,0,0,00000,icon,endTime,realduration,1.0,0);
			end
		end
		-- *** end new drain code ***
		if target then
...
function CA:CastShow(key,target)
	if bit.band(1,FW.Settings[key][0]) ~= 0 then FW:ToGroup(strformat(FW.Settings[key][1],target or "")); end
	if bit.band(2,FW.Settings[key][0]) ~= 0 then FW:ToChannel(strformat(FW.Settings[key][1],target or "")); end
	if FW.Settings[key.."Whisper"] and FW.Settings[key.."Whisper"][0] and target~=FW.PLAYER then FW:Whisper( FW.Settings[key.."Whisper"][1],target or ""); end
end
