...
function ST:RegisterManualSpellOfFlag(spell,flag)
	ManualSpell[spell] = flag;
end

local function addedByDefault(spell,flag)
	if --[[flag > ST.LAST_TIMER_FLAG or]] ManualSpell[spell] == flag then -- to show special class module flags by default
		return 1;
	...
	if frame then -- if no frame is specified this is meant to check if any non-default spells are added for show
		...
		if filter and filter[1] ~= FILTER_NONE then
			if filter[1] == FILTER_COLOR then
				if addedByDefault(spell,flag) and frame.s[ types[flag] ][0] then
					return FILTER_COLOR;
				else
					return FILTER_HIDE;
				end
...
local function ST_AddNewCooldown(spell,dura,prior,texture)
	...
	if not index or prior >= st[index][9] then
		st:remove(index);
		st:insert(dura+GetTime(),FW:CastTime(spell),dura,spell,0,COOLDOWN,texture,spell,prior,0,"none",0,PRIOR_COOLDOWN,0,1,0,0,00000,0,dura+GetTime(),dura,1.0,0);		
	end
end
...
local function ST_OnCooldownUpdate(spell,dura,flag,texture,end_time)
	...
	if flag < CD.FLAG_BUFF or flag > CD.FLAG_ENCHANT then
		...
		if index then
			...
			if FlagToAction(nil,spell,COOLDOWN) == FILTER_SHOW then
				st:insert(dura+GetTime(),FW:CastTime(spell),dura,spell,0,COOLDOWN,texture,spell,2,0,"none",0,PRIOR_COOLDOWN,0,1,0,0,00000,0,dura+GetTime(),dura,1.0,0);		
			end
		end
	end
end
...
local function ST_AddDot(unit,guid,spell,iscast)
...
function ST:AddNewTimerSimple(expire,total,targetname,spell,targettype,id,icon,guid,haste,texture,stacks,typ)--only use for non-(de)buff based adding
	...
	st:insert(expire,FW:CastTime(spell),total,targetname,0,typ,texture,spell,targettype,0,guid,ST_GetFadeTime(spell),id,0,1,stacks,0,00000,icon,expire,total,haste,0);
end
	...
	if total > 0 --[[or expire==0]] then -- dont add if duration isnt greater than 0
		...
		if found then -- found a spell i may need to remove - if it's an exact match keep using it
			if st[found][8] == spell and st[found][11] == guid and abs(st[found][1]-expire)<maxlag then 
				...
				st:remove(found);
				st:insert(expire+extratime,FW:CastTime(spell),total,targetname,Track[spell][3],Track[spell][4],Track[spell][6],spell,targettype,0,guid,ST_GetFadeTime(spell),id,0,1,stacks,0,00000,icon,expire,total,haste,0);
			end
		else
			if Track[spell][7] > 0 then
				stacks = 1;
			...
			st:insert(expire,FW:CastTime(spell),total,targetname,Track[spell][3],Track[spell][4],Track[spell][6],spell,targettype,0,guid,ST_GetFadeTime(spell),id,0,1,stacks,0,00000,icon,expire,total,haste,0);
		end
		if Track[spell][3] == 1 then
			ST_AddDot(targetname,guid,spell,true);
		end
	end	
end
...
local function ST_SpellFail(s,resist,target,typ,id,icon,guid,...)
	...
	st:insert(GetTime(),0,1,target,Track[s][3],Track[s][4],Track[s][6],s,typ,0,guid,0,id,resist,1,0,GetTime(),00000,icon,GetTime(),1,1.0,0);	
end
		...
		if debuff then
			if FlagToAction(nil,debuff,DEBUFF) == FILTER_SHOW then
				...
				elseif not st:find2(debuff,8,guid,11) then -- don't add when i already have this debuff added, but as my own cast
					...
					st:insert(expire,FW:CastTime(debuff),total,debuff,0,DEBUFF,texture,debuff,CA:Unique("target"),0,guid,0,PRIOR_DEBUFF,0,1,count or 0,0,00000,GetRaidTargetIndex("target") or 0,expire,total,1.0,0);
				end
			end
			i=i+1;
		else
		...
		if buff then
			...
			if FlagToAction(nil,buff,POWERUP) == FILTER_SHOW then
				...
				if index then
					...
					st:insert(expire,FW:CastTime(buff),total,buff,0,POWERUP,texture,buff,0,0,"none",0,PRIOR_POWERUP,0,1,(not NonStacking[buff] and count) or 0,0,00000,0,expire,total,1.0,0);
		...
		if buff then
			...
			if FlagToAction(nil,buff,SELF_DEBUFF) == FILTER_SHOW then
				...
				if index then
					...
					st:insert(expire,0,total,buff,0,SELF_DEBUFF,texture,buff,0,0,"none",0,PRIOR_SELF_DEBUFF,0,1,count or 0,0,00000,0,expire,total,1.0,0);
	...
	elseif flag2 == REMOVE then
		...
		if frame.s.HighlightColor[0] and total-v<0.5 then
			...
			elseif FW.Settings.TimerColorOverride[0] then
				...
				r,g,b = FW:MixColors((total-v)*2,frame.s.HighlightColor,frame.s[types[flag]]);
			end
		else
		 	if custom == FILTER_COLOR or custom == FILTER_SHOW_COLOR then
			...
			elseif FW.Settings.TimerColorOverride[0] then
				...
				r,g,b = unpack(frame.s[types[flag]]);
			end
		end
	end
	--alpha
...
local function NewBar(parent,n)
	...
	--	if s.RaidTargets[0] then -- hide/show done on normal update
...
local function ST_NewTimerFrame(index,root)
	...
	local function Visible(index)
		return (VisibleUnit( st[index][13], st[index][11]) and (st[index][14] < FAILED or frame.s.FailColor[0]) and FlagToAction(frame, st[index][8],st[index][6])) or FILTER_HIDE;
	end
	...
	frame.Draw = function(self)
		...
		for k=1,links.rows do -- stuff to make it 'skip' certain timers for display
			...
			if vis > 0 then -- is visible
				...
				local r,g,b,a = ColorVal(self,t1,t3,t6,t14,filter,t8);
				...
				if s.CastSpark[0] then
					...
					if t2>0 and t3>0 then -- does have a cast time
						length = t2;
						shiftme = t6 ~= COOLDOWN;
					elseif s.CastSparkGCD and t6 <= UNIQUE or t6 >= HEAL or t6==COOLDOWN then
...
function ST:RegisterMeleeBuffs()
	...
	ST:RegisterBuff(85032); -- Vicious Gladiator's Insignia of Victory, Surge of Victory
	
	ST:RegisterBuff(92213); -- Vial of Stolen Memories, Memory of Invincibility
	ST:RegisterBuff(96977); -- Matrix Restabilizer, Matrix Restabilized
	ST:RegisterBuff(96911); -- The Hungerer, Devour
	
	-- IMPORTANT DEBUFFS FROM OTHERS I WANT TO TRACK
	-- Death Knight
	ST:RegisterDebuff(96265); -- Scarlet Fever
	--warrior
...
function ST:RegisterCasterBuffs()
	...
	ST:RegisterBuff(91192); -- Mandala of Stirring Patterns, Pattern of Light
	ST:RegisterBuff(90887); -- Witching Hourglass, Witching Hour
	ST:RegisterBuff(91184); -- Fall of Mortality, Grounded Soul
	ST:RegisterBuff(100403); -- Moonwell Chalice, Blessing of the Moonwell
	...
	local function ST_CombatLogEvent(event,...)
		...
		if select(5,...) == PLAYER then
			...
			if arg2 == "SPELL_PERIODIC_DAMAGE" or arg2 == "SPELL_PERIODIC_MISSED" then
				local arg10 = select(13,...);
				if Track[arg10] and Track[arg10][3]==1 then
					ST_AddDot(select(9,...),select(8,...),arg10);
				end
			elseif arg2 == "SPELL_SUMMON" and (select(12,...) == 71843 or select(12,...) == 71844) then
				...
				st:insert(expire,0,dura,summon_valkyr,0,PET,"Interface\\Icons\\Achievement_Boss_SvalaSorrowgrave",summon_valkyr,2,0,"none",0,PRIOR_NONE,0,1, 0 ,0,00000,0,expire,dura,1.0,0);
			end
		else
...
FW:SetMainCategory(FWL.SPELL_TIMER,FW.ICON.ST,3,"TIMER",nil,"Timer",tab_actions);
	...
	FW:SetSubCategory(FWL.BASIC,FW.ICON.BASIC,2,FW.EXPAND);
		...
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.FLIP_BAR,				FWL.FLIP_BAR_TT,			"Flip",		ST_TimerShow);
	...
	FW:SetSubCategory(FWL.TIMER_FORMATS,FW.ICON.SPECIFIC,3);
		...
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.COUNTDOWN_ON_RIGHT,	FWL.COUNTDOWN_ON_RIGHT_TT,	"TimeRight",	ST_TimerShow,nil,nil,"Time");

		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_ICONS,			FWL.TIMER_ICONS_TT,			"Icon",			ST_TimerShow);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_ICONS_RIGHT,	FWL.TIMER_ICONS_RIGHT_TT,	"IconRight",	ST_TimerShow,nil,nil,"Icon");

		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_STACKS,			FWL.TIMER_STACKS_TT,		"IconStacks",	ST_TimerShow,nil,nil,"Icon");
		FW:RegisterOption(FW.FNT,2,FW.LAS,FWL.ICON_FONT,			"",							"StacksFont",	ST_TimerShow,nil,nil,function(obj) return obj.s.Icon and obj.s.IconStacks; end);

	FW:SetSubCategory(FWL.NAMING_GROUPING,FW.ICON.SPECIFIC,3);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.DISPLAY_MODES7,	FWL.DISPLAY_MODES7_TT,	"GroupID");
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.DISPLAY_MODES10,	FWL.DISPLAY_MODES10_TT,	"Label",		nil,nil,nil,"GroupID");
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.FLIP_TAG,			FWL.FLIP_TAG_TT,		"LabelFlip",	ST_TimerShow,nil,nil,function(obj) return obj.s.GroupID and obj.s.Label; end );
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.LABEL_LIMIT,		FWL.LABEL_LIMIT_TT,		"LabelLimit",	ST_TimerShow,nil,nil,function(obj) return obj.s.GroupID and obj.s.Label; end );
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_BAR_LABELS,	FWL.TIMER_BAR_LABELS_TT,"Name",			ST_TimerShow);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.DISPLAY_MODES9,	FWL.DISPLAY_MODES9_TT,	"Spell",		nil,nil,nil,"Name");
		FW:RegisterOption(FW.MSG,1,FW.NON,FWL.CUSTOM_TAG,		FWL.CUSTOM_TAG_TT,		"CustomTag",	nil,nil,nil,"Name");
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.DISPLAY_MODES8,	FWL.DISPLAY_MODES8_TT,	"ShowID",		nil,nil,nil,"Name");
	...
	FW:SetSubCategory(FWL.SPARKS_AND_GLOW,FW.ICON.GLOW,3);
		...
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.SPARK_TICK,		FWL.SPARK_TICK_TT,									"SparkDamage",	ST_TimerShow,1,2,"Spark");
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.CAST_GLOW,		FWL.CAST_GLOW_TT..FWL._EDITBOX_TRANSPARENCY,		"CastSpark",	ST_TimerShow,0,1);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.CAST_OVERLAP,		FWL.CAST_OVERLAP_TT,								"CastSparkTickOverlap",	nil,nil,nil,"CastSpark");
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.CAST_GCD,			FWL.CAST_GCD_TT,									"CastSparkGCD",		nil,nil,nil,"CastSpark");
		...
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TICKS_NEXT,		FWL.TICKS_NEXT_TT,									"TicksNext",	ST_HideTicks,nil,nil,"Ticks");
	...
	FW:SetSubCategory(FWL.TIMER_MAXIMUM_TIME,FW.ICON.TIME,3)
		...
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.FORCEMAX,			FWL.FORCEMAX_TT,		"ForceMax",			nil,nil,nil,"MaxTime");
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.DISPLAY_TYPES3,	FWL.DISPLAY_TYPES3_TT,	"HideLonger",		nil,0);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.DISPLAY_TYPES11,	FWL.DISPLAY_TYPES11_TT,	"HideLongerNoBoss",	nil,nil,nil,"HideLonger");
		FW:RegisterOption(FW.NUM,1,FW.NON,FWL.DISPLAY_TYPES4,	FWL.DISPLAY_TYPES4_TT,	"HideTime",			nil,0,nil,"HideLonger");
	...
	FW:SetSubCategory(FWL.FADING,FW.ICON.FADE,3)
		...
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.EXPIRED,			FWL.EXPIRED_TT,			"ExpiredColor"		);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.HIGHLIGHT,		FWL.FADING5_TT,			"HighlightColor"		);

	FW:SetSubCategory(FWL.TIMER_UNITS,FW.ICON.UNITS,6)
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_RAID_DEBUFFS,FWL.TIMER_RAID_DEBUFFS_TT,	"RaidDebuffs",	ST_TimerFilterChange);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_SHOW_TARGET,FWL.TIMER_SHOW_TARGET_TT,	"Target",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_SHOW_FOCUS,	FWL.TIMER_SHOW_FOCUS_TT,	"Focus",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_SHOW_OTHER,	FWL.TIMER_SHOW_OTHER_TT,	"Other",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_SHOW_UKNOWN,FWL.TIMER_SHOW_UKNOWN_TT,	"UnknownTarget",ST_TimerFilterChange);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.SHOW_WITHOUT_UNIT,FWL.SHOW_WITHOUT_UNIT_TT,	"NoTarget",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CHK,1,FW.NON,FWL.TIMER_SHOW_YOU,	FWL.TIMER_SHOW_YOU_TT,		"You",			ST_TimerFilterChange);
	...
	FW:SetSubCategory(FWL.MY_SPELLS,FW.ICON.FILTER,7,nil,FW.REDUCED_ALPHA);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.NORMAL_TYPE,		FWL.NORMAL_TYPE_TT,		"MagicColor",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.SHARED_TYPE,		FWL.SHARED_TYPE_TT,		"CurseColor",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.SHARED_TYPE2,		FWL.SHARED_TYPE2_TT,	"Shared2Color",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.SHARED_TYPE3,		FWL.SHARED_TYPE3_TT,	"Shared3Color",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.UNIQUE_TYPE,		FWL.UNIQUE_TYPE_TT,		"CrowdColor",		ST_TimerFilterChange);
		
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.PET,				FWL.DISPLAY_TYPES10_TT,	"PetColor",			ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.FRIENDLY_BUFFS,	FWL.DISPLAY_TYPES9_TT,	"FriendlyBuffColor",	ST_TimerFilterChange);	
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.HEAL,				FWL.DISPLAY_TYPES8_TT,	"HealColor",			ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.DRAIN,			FWL.DRAIN_TT,			"DrainColor",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.FAIL,				FWL.DISPLAY_TYPES5_TT,	"FailColor",			ST_TimerFilterChange);
	
	FW:SetSubCategory(FWL.MY_COOLDOWNS,FW.ICON.FILTER,7,nil,FW.REDUCED_ALPHA);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.COOLDOWNS,		FWL.COOLDOWNS_TT,		"CooldownsColor",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.ALL_OTHER_COOLDOWNS,		"",				"CooldownsOtherColor",	ST_TimerFilterChange,nil,nil,false);
		
	FW:SetSubCategory(FWL.BUFFS_DEBUFFS_ON_ME,FW.ICON.FILTER,7,nil,FW.REDUCED_ALPHA);	
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.POWERUP_BUFFS,	FWL.DISPLAY_TYPES1_TT,	"BuffColor",				ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.ALL_OTHER_BUFFS,			"",				"BuffOtherColor",		ST_TimerFilterChange,nil,nil,false);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.DEBUFFS,			FWL.DEBUFFS_TT,			"SelfDebuffColor",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.ALL_OTHER_DEBUFFS,			"",			"SelfDebuffOtherColor",	ST_TimerFilterChange,nil,nil,false);

	FW:SetSubCategory(FWL.RAID_DEBUFFS,FW.ICON.FILTER,7,nil,FW.REDUCED_ALPHA);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.OTHER_DEBUFFS,	FWL.OTHER_DEBUFFS_TT,	"DebuffsColor",		ST_TimerFilterChange);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.ALL_OTHER_DEBUFFS,			"",			"DebuffsOtherColor",	ST_TimerFilterChange,nil,nil,false);
	...
	FW:SetSubCategory(FWL.APPEARANCE,FW.ICON.APPEARANCE,9);	
		FW:RegisterOption(FW.COL,1,FW.NON,FWL.NORMAL_TEXT,	FWL.NORMAL_TEXT_TT,	"NormalColor",	ST_TimerShow);
		FW:RegisterOption(FW.COL,1,FW.NON,FWL.FRAME_BACKGROUND,	"",				"NormalBgColor",	ST_TimerShow);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.TARGET_TEXT,	FWL.TARGET_TEXT_TT,	"TargetColor");
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.FOCUS_TEXT,	FWL.FOCUS_TEXT_TT,	"FocusColor");		
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.TARGET_BACKGROUND,"",				"TargetBgColor",	ST_TimerShow);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.FOCUS_BACKGROUND,	"",				"FocusBgColor",	ST_TimerShow);
...
FW:SetMainCategory(FWL.SELF_MESSAGES);
	FW:SetSubCategory(FWL.SPELL_TIMER,FW.ICON.DEFAULT,2);
		FW:RegisterOption(FW.CO2,1,FW.NON,FWL.SHOW_FAILED,	FWL.SHOW_FAILED_TT,"TimerResistsColor");
...
FW:SetMainCategory(FWL.ADVANCED,FW.ICON.DEFAULT,99,"DEFAULT");
	FW:SetSubCategory(FWL.SPELL_TIMER,FW.ICON.DEFAULT,3);
		...
		FW:RegisterOption(FW.NU2,1,FW.NON,FWL.SMART_COUNTDOWN_WIDTH,FWL.SMART_COUNTDOWN_WIDTH_TT,"TimerSmartSpace",nil,1,60);
		...
		FW:RegisterOption(FW.CO2,1,FW.NON,"Type color override","",		"TimerColorOverride");
...
FW.InstanceDefault.Timer = {
	...
	DrainColor = 			{[0]=true,0.42,0.00,1.00},

	CurseColor = 			{[0]=true,0.64,0.21,0.93},
	Shared2Color = 			{[0]=true,0.00,0.54,0.42},
	Shared3Color = 			{[0]=true,1.00,0.50,0.00},

	CrowdColor = 			{[0]=true,0.00,1.00,0.50},
	MagicColor = 			{[0]=true,1.00,0.50,0.00},
	PetColor = 				{[0]=true,1.00,0.00,0.95},
	HealColor = 			{[0]=true,0.00,1.00,0.00},
	FriendlyBuffColor = 	{[0]=true,1.00,1.00,0.00},
	HighlightColor =		{[0]=true,1.00,1.00,1.00},
	FailColor =				{[0]=true,1.00,0.00,0.30},

	BuffColor = 			{[0]=true,0.00,0.75,1.00},
	BuffOtherColor = 		{[0]=false,0.50,0.50,0.50},
	DebuffsColor = 			{[0]=true,0.00,0.36,1.00},
	DebuffsOtherColor = 	{[0]=false,0.00,0.18,0.50},
	CooldownsColor =		{[0]=true,1.00,0.39,0.35},
	CooldownsOtherColor =	{[0]=false,0.50,0.20,0.18},
	SelfDebuffColor =		{[0]=true,1.00,0.00,0.00},
	SelfDebuffOtherColor =	{[0]=false,0.50,0.00,0.00},
